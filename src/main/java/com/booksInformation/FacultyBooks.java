...
import com.register.ConnectionProcess;
	...
	public class FacultyBooks {
		...
		public static void TakeBook(int Faculty) throws Exception
		{
		...
		public static void addId(int Faculty_id,int name1) throws Exception
		{
			String name2="not return";
			LocalDate date=LocalDate.now();
			String query1="insert into book_register (staff_id,book_id,book_taken_date,book_status)values('"+Faculty_id+"','"+name1+"','"+date+"','"+name2+"')" ;
			Connection con = ConnectionProcess.databaseconnection();
			Statement stm = con.createStatement();
			stm.executeUpdate(query1);
		}
			
			public static void ReturnBook(int faculty) throws Exception
		  {
		...
		public static void TodayNews() throws Exception
		{
			...
			Connection connectionn = ConnectionProcess.databaseconnection();
			String query1 = "SELECT Indian_Express,Hindu,Dinamalar FROM Daily_News_Papers where News_Date = '"+ number1+"'";
			Statement st = connectionn.createStatement();
			ResultSet rs = ((java.sql.Statement) st).executeQuery(query1);
			while (rs.next()) {
				System.out.println("Indian Expression  -->"+rs.getString("Indian_Express"));
				System.out.println("The Hindu  -->"+rs.getString("Indian_Express"));
				System.out.println("Dinamalar  -->"+rs.getString("Indian_Express"));
	}
